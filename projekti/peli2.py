...
def paivita_sijainti(valittu_lentokentta, etaisyys_uuteen, pelaajan_sijainti, kilsat_pelaaja, pelaajan_aika, lentokentat):
    # Päivitetään pelaajan sijainti (ICAO-koodi)
    ...
    # Haetaan lentokentän nimi ICAO-koodin perusteella
    lentokentan_nimi = next((airport['name'] for airport in lentokentat if airport['ident'] == pelaajan_sijainti), None)

    # Tulostetaan päivitetyt tiedot
    print(f"\nNykyinen sijainti: {lentokentan_nimi}")
    print(f"Jäljellä olevat kilometrit: {kilsat_pelaaja:.2f} km")
    print(f"Jäljellä oleva aika: {pelaajan_aika // 60:.0f} tuntia ja {pelaajan_aika % 60:.0f} minuuttia.")
